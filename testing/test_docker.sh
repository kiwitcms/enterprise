...
rlJournalStart
    ...
    rlPhaseStartTest "Authenticated GET /uploads/ cross tenant returns 403"
        ...
        rlRun -t -c "curl -k -D- --referer authenticated_cross_tenant -b ./unauthorized-cookies.txt --silent $HTTPS/uploads/tenant/empty/attachments/testplans_testplan/1/hello-robots.txt | grep '404 Not Found'"
    rlPhaseEnd
    ...
    rlPhaseStartTest "Requests for uploaded files are rate limited"
        ...
        COMPLETED_REQUESTS=$(exec_wrk "$HTTPS/uploads/tenant/public/attachments/auth_user/2/ldap.py" "$WRK_DIR" "uploaded-file" "Cookie: sessionid=$SESSION_ID")
        rlLogInfo "COMPLETED_REQUESTS=$COMPLETED_REQUESTS in 10 seconds"
